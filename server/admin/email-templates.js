...
Accounts.emailTemplates.from = 'Bloom \<obsidian.orchid.bloom@gmail.com>';

//verifying account
Accounts.emailTemplates.verifyEmail.subject = function(user) {
  return '[Bloom] Verify email address for ' + user.profile.firstName + ' ' + user.profile.lastName;
};
Accounts.emailTemplates.verifyEmail.text = function(user, url){
  var address = user.emails[0].address;
  var url = url.replace('#/', '');
  var from = 'obsidian.orchid.bloom@gmail.com';
  var body = 'Thank you for registering an account with Bloom! Please click the' +
...
Accounts.emailTemplates.resetPassword.subject = function(user) {
  return '[Bloom] Reset password request by ' + user.profile.firstName + ' ' + user.profile.lastName;
};
Accounts.emailTemplates.resetPassword.text = function(user, url){
  var address = user.emails[0].address;
  var url = url.replace('#/', '');
  var from = 'obsidian.orchid.bloom@gmail.com';
  var body = 'You requested a password reset for your Bloom account. To complete' +
